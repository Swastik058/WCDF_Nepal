    ...
    <div className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">
      ...
      <div className="space-y-4 p-6">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h3 className="text-xl font-semibold text-slate-900">{child.fullName}</h3>
            <p className="mt-1 text-sm text-slate-500">{child.age ? `${child.age} years old` : 'Age unavailable'}</p>
          </div>
          <span className="rounded-full bg-emerald-100 px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] text-emerald-700">
            {child.joinedYear ? `Joined ${child.joinedYear}` : 'Residing'}
          </span>
        </div>
        <p className="min-h-[3.5rem] text-sm leading-6 text-slate-600">{child.shortBio || 'No story available yet.'}</p>
